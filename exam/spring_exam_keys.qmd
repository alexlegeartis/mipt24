...
1.  $A$-сопряженность двух векторов. $A$-ортогональность. Скалярное произведение $\langle \cdot, \cdot \rangle_A$.
    ...
    Замечание: $\langle \cdot, \cdot \rangle_A$ корректней называть полускалярным произведением (semi-inner product), поскольку в общем случае оно лишь неотрицательно определено. Однако если $A$ симметрична и положительно определена, то $\langle \cdot, \cdot \rangle_A$ будет настоящим скалярным произведением, т.е. положительно определённым (это следует из т. Гильберта-Шмидта и ортогональности собственных векторов, соответствующих различным собственным числам).
    :::
